    ...
    edited_incomes_df = st.data_editor(
        ...
        column_config={
            ...
            "預計收入日期": st.column_config.DateColumn(
                "第三項：預計收入日期 (點擊彈出日曆)",
                min_value=datetime.date.today(), # 限制不能選過去的日期
                format="YYYY-MM-DD"
            ),
...
def simulate_timeline(target_val, initial_savings, monthly_savings, active_inc_df):
    """模擬每日現金流，精確找出加入特定日期收入後，在哪一天能達成目標"""
...
if base_savings <= 0:
    ...
    with graph_col2:
        ...
        if selected_marginal and "selection" in selected_marginal and selected_marginal["selection"]["points"]:
            ...
            st.caption("提示：點擊上方柱狀圖的長條，可在這裡動態查看該區間的詳細邊際效益。")import streamlit as st
import pandas as pd
import plotly.express as px
import datetime
