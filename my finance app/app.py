...
# ---- 2. 目前個人總存款 (單獨一行) ----
st.markdown("### **2. 目前個人總存款 (元)**")
savings = st.number_input("", min_value=0, value=1000, step=100, label_visibility="collapsed", key="savings_input")
st.write("") 
...
with st.expander("3. 點開填寫：每月必定消費的開支 (剛需開支)", expanded=True):
    ...
    total_essential = pd.to_numeric(edited_essential_df["消費金額"]).sum()
    essential_total_placeholder.metric("必定消費總計", f"{total_essential} 元")

st.write("")

# ---- 4. 每月自我意願消費（非剛需）摺疊區 (單獨一行，已刪除提示字) ----
with st.expander("4. 點開填寫：每月自我意願消費 (娛樂開支)", expanded=True):
    ...
    total_discretionary = pd.to_numeric(edited_discretionary_df["消費金額"]).sum()
    discretionary_total_placeholder.metric("自我意願總計", f"{total_discretionary} 元")
...
if base_savings <= 0:
    ...
    base_months = target_value / base_savings
    ...
    for i, s in enumerate(s_percentages):
        ...
        new_months = target_value / new_savings
